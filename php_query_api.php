...
function time_series_query_meteocache(
    DateTime $start_date,
    DateTime $end_date,
    $resolution,
    array $parameters,
    ...
    if (sizeof($parameters) > 0) {
        ...
        $url = "{$DEFAULT_API_BASE_URL}/{$start_date_str}--{$end_date_str}:{$resolution}/{$model}/" . "{$lat},{$lon}/{$format}";
        // $url = "{$DEFAULT_API_BASE_URL}/{$start_date_str}--{$end_date_str}:{$resolution}/" . "{$lat},{$lon}/{$format}?model={$model}";
    }
    // $parameters_str = explode($parameters, ",");
...
echo time_series_query_meteocache($start, $end, 'PT12H', [], 't_2m:C', 4.3556, 74.0451, 'html_map');
